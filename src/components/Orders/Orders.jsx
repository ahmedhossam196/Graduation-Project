...
const OrderCard = ({ order, deliveryMethods }) => {
  const deliveryId =
    order.deliveryMethodId || order.deliveryMethod?.id || order.deliveryId;

  const matchedMethod = deliveryMethods.find(
    (m) =>
      String(m.id) === String(deliveryId) ||
      m.shortName === order.deliveryMethod
  );

  const deliveryDisplay =
    matchedMethod?.deliveryTime ||
    order.deliveryMethod?.deliveryTime ||
    order.deliveryTime ||
    "Standard Shipping";

  const deliveryCost =
    typeof order.shippingPrice !== "undefined"
      ? order.shippingPrice
      : matchedMethod
      ? matchedMethod.price
  ...
  const pMethod =
    order.paymentMethod || (order.address && order.address.paymentMethod);
  ...
  const finalTotal = order.total || itemsTotal + deliveryCost;

  const rawDate =
    order.orderdate || order.orderDate || order.createdAt || "N/A";
  ...
  const getStatusStep = (status) => {
    ...
    if (s === "pending" || s === "ordered" || s === "processing" || s === "1")
      return 1;
    if (s === "shipped" || s === "in route" || s === "2") return 2;
    if (s === "delivered" || s === "completed" || s === "3") return 3;
    return 1;
  };

  const statusStep = order.statusStep || getStatusStep(order.status);
  ...
  return (
    <div className="bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-700 rounded-2xl shadow-sm p-6 mb-8">
      <div className="flex flex-col lg:flex-row justify-between items-start lg:items-center border-b pb-6 mb-6 gap-4">
        <div className="flex-grow text-left">
          ...
          <div className="p-4 rounded-xl border border-gray-200 dark:border-gray-700 mb-4 text-left">
            <p className="text-xs font-bold text-[#009DDC] uppercase mb-1">
              Shipping to:
            </p>
          ...
          <div className="flex flex-wrap gap-2">
            ...
            >
              <i
                className={`fa-solid ${
                  isCash ? "fa-money-bill-wave" : "fa-credit-card"
                } mr-1`}
              ></i>{" "}
              {paymentLabel}
            </span>
        ...
        <div className="bg-white dark:bg-gray-800 p-5 rounded-xl border border-gray-200 dark:border-gray-700 min-w-[240px]">
          ...
          <div className="border-t pt-3 flex justify-between items-center">
            ...
            <span className="text-2xl font-black text-[#009DDC]">
      ...
      <div className="space-y-3 text-left">
        <h4 className="text-xs font-black mb-2 uppercase tracking-widest text-gray-400">
          Order Contents
        </h4>
...
export default function Orders() {
  ...
  const displayedOrders =
    viewMode === "last" && orders.length > 0 ? [orders[0]] : orders;
        ...
        {orders.length > 0 && (
          <div className="flex bg-white dark:bg-gray-800 p-1.5 rounded-2xl shadow-sm border border-gray-200 dark:border-gray-700">
            <button
              ...
              className={`px-6 py-2.5 rounded-xl cursor-pointer text-sm font-bold transition-all duration-300 ${
                viewMode === "last"
