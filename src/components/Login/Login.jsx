...
export default function Login() {
  ...
  function handleLogin(data){
    console.log(data);
    setisLoading(true)
    axios.post(`http://smartbracelet.runasp.net/api/auth/login`, data).then((res)=>{
      console.log(res);
    ...
    <div className="grid lg:grid-cols-2 items-center gap-6 max-w-6xl w-full">
      <div className="border border-slate-300 rounded-lg p-6 max-w-md shadow-[0_2px_22px_-4px_rgba(93,96,127,0.2)] max-lg:mx-auto">
        ...
        <form className="space-y-6" onSubmit={handleSubmit(handleLogin)}>
          ...
          <div className="mb-12">
            <h1 className="text-[#009DDC]  text-3xl font-semibold">Sign in</h1>
            <p className="text-slate-600 text-[15px] mt-6 leading-relaxed">Log in and discover how SmartCare keeps you and your loved ones protected.</p>
          </div>
          <div>
            <label className="text-slate-900 text-sm font-medium mb-2 block">Email</label>
            <div className="relative flex items-center">
              <input {...register("email")} type="email" required className="w-full text-sm text-slate-900 border border-slate-300 pl-4 pr-10 py-3 rounded-lg outline-blue-600" placeholder="Enter Email" />
          ...
          <div>
            ...
            <div className="relative flex items-center">
              <input {...register("password")} type="password" required className="w-full text-sm text-slate-900 border border-slate-300 pl-4 pr-10 py-3 rounded-lg outline-blue-600" placeholder="Enter password" />
          ...
          <div className="flex flex-wrap items-center justify-between gap-4">
            <div className="flex items-center">
              <input id="remember-me" name="remember-me" type="checkbox" className="h-4 w-4 shrink-0 text-[#009DDC] focus:ring-blue-500 border-slate-300 rounded" />
              <label htmlFor="remember-me" className="ml-3 block text-sm text-slate-900">
                Remember me
              </label>
            </div>
            <div className="text-sm">
              <Link to="forgetPassword" className="text-[#009DDC] hover:underline font-medium">
          ...
          <div className="!mt-12">
            <button 
                disabled={isLoading}
            type="submit" className="w-full shadow-xl py-2.5 px-4 text-[15px] font-medium tracking-wide rounded-lg text-white bg-[#009DDC] hover:bg-blue-600 focus:outline-none cursor-pointer">
      {isLoading ? <i className='fas fa-spinner fa-spin text-white'></i> :"Sign in"}
            </button>
      ...
      <div className="max-lg:mt-8">
        <img src={loginpic} className="w-full aspect-[71/50] max-lg:w-4/5 mx-auto block object-cover" alt="login img" />
      </div>
    </div>
  </div>
</div>
